...
fn open_url(url: String) -> Result<(), String> {
  ...
  {
    // `cmd /C start` kalıbı tırnaklanmayan URL'de `&` sonrasını cmd'ye yediriyor
    // (OAuth redirect_to kaybı). rundll32 kabuk yorumlaması yapmadan ShellExecute
    // çağırır; `&` ve `%` bozulmadan geçer.
